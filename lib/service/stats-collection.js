...
const SpareAmount = require('../chia-amount');
...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'spare-dashboard-satellite';
  ...
  async updateWalletStats() {
    ...
    walletStats.wallets = await Promise.all(wallets.map(async wallet => {
      ...
      return {
        ...
        type: wallet.type,
        balance: {
          confirmed: SpareAmount.fromRaw(balance.confirmed_wallet_balance).toString(),
          spendable: SpareAmount.fromRaw(balance.spendable_balance).toString(),
          unconfirmed: SpareAmount.fromRaw(balance.unconfirmed_wallet_balance).toString(),
        },
      };
    }));
    ...
    walletStats.farmedAmount = {
      farmedAmount: SpareAmount.fromRaw(farmedAmount.farmed_amount).toString(),
      poolRewardAmount: SpareAmount.fromRaw(farmedAmount.pool_reward_amount).toString(),
      farmerRewardAmount: SpareAmount.fromRaw(farmedAmount.farmer_reward_amount).toString(),
      feeAmount: SpareAmount.fromRaw(farmedAmount.fee_amount).toString(),
